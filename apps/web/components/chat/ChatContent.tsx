...
const MODEL_CONFIGS: Record<ModelType, { label: string; icon: any; color: string }> = {
    ...
    budget: { label: "Budget", icon: Wallet, color: "text-pink-500" },
    research: { label: "Research", icon: BookOpen, color: "text-indigo-500" },
};
...
export default function ChatContent({
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (conversation.model === "research") {
                await handleResearch(input, updatedMessages);
                return;
            }

            if (conversation.model === "budget") {
                setShowBudget(true);
                setIsLoading(false);
                return;
            }
    ...
    const handleResearch = async (topic?: string, baseMessages?: ChatMessage[]) => {
        const researchTopic = topic || prompt.trim();
        ...
        try {
            ...
            const assistantMsg: ChatMessage = {
                id: (Date.now() + 1).toString(),
                role: "assistant",
                content: `### Research Document Generated\n\nI have synthesized a comprehensive research document for you regarding **${researchTopic}**. You can download the compiled PDF report below.`,
                pdfUrl: pdfUrl
            };

            console.log("Research PDF URL:", pdfUrl);
            updateConversation(conversation.id, {
                messages: [...(baseMessages || conversation.messages), assistantMsg]
            });
            setPrompt("");
        } catch (err: any) {
    ...
    return (
        <main className="flex h-screen flex-col overflow-hidden bg-background">
            ...
            <div className="relative flex-1 overflow-hidden">
                <ChatContainerRoot className="h-full overflow-x-hidden">
                    <ChatContainerContent className="space-y-0 py-8 max-w-full overflow-x-hidden">
                        ...
                        {conversation.messages.map((message) => {
                            ...
                            return (
                                <Message
                                    key={message.id}
                                    className={cn(
                                        "mx-auto flex w-full max-w-3xl flex-col gap-2 px-4 sm:px-6 py-4 animate-in fade-in slide-in-from-bottom-2 overflow-hidden",
                                ...
                                >
                                    {isAssistant ? (
                                                ...
                                                {message.pdfUrl && (
                                                    <div className="mt-4 p-4 rounded-xl bg-orange-500/5 border border-orange-500/20 flex items-center justify-between group/pdf">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <div className="text-sm font-bold opacity-90">Research Report.pdf</div>
                                                                <div className="text-[10px] opacity-50 uppercase font-mono tracking-tighter">Generated Document</div>
                                                            </div>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            <Button
                                                                variant="outline"
                                                                size="sm"
                                                                className="h-8 rounded-full text-[10px] bg-background"
                                                                onClick={() => window.open(`${SERVER_URL}${message.pdfUrl}`, '_blank')}
                                                            >
                                                                View
                                                            </Button>
                                                            <Button
                                                                size="sm"
                                                                className="h-8 rounded-full text-[10px] bg-orange-600 hover:bg-orange-700"
                                                                onClick={() => {
                                                                    const link = document.createElement('a');
                                                                    link.href = `${SERVER_URL}${message.pdfUrl}`;
                                        ...
                                        <div className="group flex flex-col items-end gap-1 w-full max-w-full overflow-hidden">
                                            <MessageContent className="bg-primary text-primary-foreground max-w-[85%] rounded-2xl px-4 py-2 sm:max-w-[75%] border-none min-w-[80px] w-fit break-words">
                                                {message.content}
                                            </MessageContent>
                                        </div>
                                    )}
