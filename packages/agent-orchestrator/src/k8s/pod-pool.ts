...
declare const Bun: {
  spawnSync: (cmd: string[]) => { exitCode: number; stdout: Buffer | string; stderr: Buffer | string }
}
...
function getStatusMessage(status: V1Status): string | undefined {
  const causeMessages = status.details?.causes
    ?.map((cause: { message?: string }) => cause.message)
...
function getServiceAccountToken(namespace: string): string | undefined {
  if (typeof Bun === "undefined" || !Bun.spawnSync) return undefined
...
function runKubectlExec(namespace: string, podName: string, command: string): string {
  if (typeof Bun === "undefined" || !Bun.spawnSync) {
    throw new Error("Bun.spawnSync is not available in current runtime environment")
  }
...
export class PodPool {
  ...
  async getPods(): Promise<Pod[]> {
    ...
    return podList.items
      ...
      .filter((podName: string | undefined): podName is string => Boolean(podName))
      .sort()
      .map((podName: string) => ({
